...
def train_model_simple(
    ...
    from generation import generate_and_print_simple
...
def calc_loss_loader_classifier(dataloader, model, device, num_batches=None, num_classes=2):
    total_loss = 0
...
def train_classifier_simple(model, train_loader, val_loader, device, optimizer, eval_freq, eval_iter, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, target_batch in train_loader:
            optimizer.zero_grad()
            loss = calc_loss_batch_classifier(input_batch, target_batch, model, device, num_classes=2)
            loss.backward()
            optimizer.step()
