...
GPT2_SIZE_MAP = {"small": "124M", "medium": "355M", "large": "774M", "xlarge": "1558M"}
...
def load_weights_into_classifier(classifier, params, num_classes=2):
    ...
    for b in range(len(params["blocks"])):
        ...
        classifier.trf_blocks[b].norm1.scale = assign(
            classifier.trf_blocks[b].norm1.scale, params["blocks"][b]["ln_1"]["g"]
        )
        classifier.trf_blocks[b].norm1.shift = assign(
            classifier.trf_blocks[b].norm1.shift, params["blocks"][b]["ln_1"]["b"]
        )

        classifier.trf_blocks[b].norm2.scale = assign(
            classifier.trf_blocks[b].norm2.scale, params["blocks"][b]["ln_2"]["g"]
        )
        classifier.trf_blocks[b].norm2.shift = assign(
            classifier.trf_blocks[b].norm2.shift, params["blocks"][b]["ln_2"]["b"]
        )
...
def load_pretrained_gpt2(model_size="small", models_dir="gpt2", device="cpu"):
    ...
    settings, params = download_and_load_gpt2(
        model_size=GPT2_SIZE_MAP[model_size], models_dir=models_dir
    )
