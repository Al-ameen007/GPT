...
def generate_text(prompt, model, tokenizer, device,max_new_tokens=100, temperature=0.7, top_k=50):
    model.eval()
...
def classify_text(text, model, tokenizer, device, max_length, pad_token_id=50256, return_probs=False):
    model.eval()
    input_ids = tokenizer.encode(text)
    supported_context_length = model.pos_emb.weight.shape[0]
